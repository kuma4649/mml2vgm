...
namespace Core
{
    public class SN76489X2 : ClsChip
    {
        ...
        public SN76489X2(ClsVgm parent, int chipID, string initialPartName, string stPath, int chipNumber) : base(parent, chipID, initialPartName, stPath, chipNumber)
        {
            _Name = "SN76489X2";
            _ShortName = "DCSGX2";
            _ChMax = 4;
        ...
        private void OutPsgFnum(partPage page, MML mml)
        {
            ...
            if (page.Type != enmChannelType.DCSGNOISE)
            {
                SOutData(page, mml, port[0]
                    , (byte)(0x20 + (page.ch << 2) + ((page.freq & 0x300) >> 8))
                    , (byte)page.freq);
            }
            else
            {
        ...
        {
            if (page.pcm) return;
            if (page.spg.beforeVolume == page.beforeVolume) return;

            page.spg.beforeVolume = page.beforeVolume;
            OutPsgPort(page, mml, port[0]
                , (byte)(0x80 + (page.ch << 4) + (15 - page.beforeVolume))
                );
        }
        ...
        public override void CmdY(partPage page, MML mml)
        {
            ...
            msgBox.setErrMsg("XGM2はyコマンド未サポートです",null);
        }
        ...
        public override void MultiChannelCommand(MML mml)
        {
            ...
            {
                //if (parent.info.format != enmFormat.XGM && parent.info.format != enmFormat.XGM2)//XGMではGGのステレオ機能は対応していない
                //{
