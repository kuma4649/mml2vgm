...
{
    [System.Diagnostics.CodeAnalysis.SuppressMessage("Usage", "CA2211:非定数フィールドは表示されません", Justification = "<保留中>")]
    public class Audio
    {
        ...
        public static Manager mmlParams = new();
        ...
        public static HashSet<EnmChip> useChip = [];
...
#pragma warning restore CS0414 // フィールド 'Audio.bufVirtualFunction_MIDIKeyboard' が割り当てられていますが、値は使用されていません。
        ...
        private static List<NAudio.Midi.MidiOut> midiOuts = [];
        private static List<int> midiOutsType = [];
        ...
        public static void RealChipAutoDetect(Setting setting)
        {
            ...
            chipType = new Setting.ChipType[Math.Max(chipRegister.K054539.Count, 2)];
            for (int i = 0; i < Math.Max(chipRegister.K054539.Count, 2); i++)
            {
                chipType[i] = new Setting.ChipType();
                if (chipRegister.K054539.Count <= i) continue;
                if (!chipRegister.K054539[i].Use) continue;
                chipRegister.K054539[i].Model = EnmVRModel.VirtualModel;
                chipType[i].UseEmu = true;
                chipType[i].UseScci = false;
                if (ret.Count == 0) continue;
            }
            chipRegister.SetRealChipInfo(EnmZGMDevice.K054539, chipType[0], chipType[1], setting.LatencyEmulation, setting.LatencySCCI);
        ...
        private static void SoundManagerMount()
        {
            ...
            DriverAction DriverAction = new()
            {
                Init = DriverActionInit,
                Main = DriverActionMain,
                Final = DriverActionFinal
            };
            sm.setting = setting;
            sm.Setup(
        ...
        private static bool playSet()
        {
            ...
            else if (PlayingFileFormat == EnmFileFormat.XGM2)
            {
                driver = new xgm2
                {
                    setting = setting
        ...
        public static bool xgmPlay(Setting setting)
        {
            ...
            try
            {
                ...
                if (!driver.init(vgmBuf, chipRegister, [EnmChip.YM2612, EnmChip.SN76489]
        ...
        public static bool xgm2Play(Setting setting)
        {
            ...
            try
            {
                ...
                List<byte> pcmBuf = [];
                for (int i = 0; i < ((xgm2)driver).sampleDataBlockSize ; i++)
                {
                    pcmBuf.Add(vgmBuf[((xgm2)driver).sampleDataBlockAddr + i].val);
                }
                ...
                chip.SamplingRate = (UInt32)Common.SampleRate;
                chip.Volume = setting.balance.YM2612Volume;
                chip.Clock = (uint)xgm2Driver.YM2612ClockValue;
                chip.Option = null;
                chipLED.PriOPN2 = 1;
                chipRegister.YM2612[0].Use = true;
                lstChips.Add(chip);
                ...
                chip.SamplingRate = (UInt32)Common.SampleRate;
                chip.Volume = setting.balance.SN76489Volume;
                chip.Clock = (uint)xgm2Driver.SN76489ClockValue;
                chip.Option = null;
                ...
                if (setting.YM2612Type.UseEmu)
                {
                    ym2612.XGMfunction.sampleID[0] = ((xgm2)driver).sampleID;
                    ym2612.XGMfunction.xgm2pcm[0] = ((xgm2)driver).xgm2pcm;
                    ym2612.XGMfunction.pcmBuf[0] = [.. pcmBuf];
                }
                else if (setting.YM2612Type.UseEmu2)
                {
                    ym3438.XGMfunction.sampleID[0] = ((xgm2)driver).sampleID;
                    ym3438.XGMfunction.xgm2pcm[0] = ((xgm2)driver).xgm2pcm;
                    ym3438.XGMfunction.pcmBuf[0] = [.. pcmBuf];
                }
                else if (setting.YM2612Type.UseEmu3)
                {
                    ym2612mame.XGMfunction.sampleID[0] = ((xgm2)driver).sampleID;
                    ym2612mame.XGMfunction.xgm2pcm[0] = ((xgm2)driver).xgm2pcm;
                    ym2612mame.XGMfunction.pcmBuf[0] = [.. pcmBuf];
                }
        ...
        public static bool zgmPlay(Setting setting)
        {
            ...
            try
            {
                ...
                if (!driver.init(vgmBuf
                    , chipRegister
                    , [EnmChip.YM2203]// usechip.ToArray()
                    , (uint)(Common.SampleRate * setting.LatencyEmulation / 1000)
                    , (uint)(Common.SampleRate * setting.outputDevice.WaitTime / 1000)
                    , jumpPointClock
                    ))
                ...
                log.Write("使用チップの調査");
                {
                    ...
                    foreach (Driver.ZGM.ZgmChip.ZgmChip zchip in zgmDriver.chips)
                    {
                        ...
                        chip = new MDSound.MDSound.Chip
                        {
                            ...
                            Clock = (uint)zchip.defineInfo.clock,
                            Option = [(int)zchip.defineInfo.option[0]]
                        };
                        lstChips.Add(chip);
                    ...
                    foreach (Driver.ZGM.ZgmChip.ZgmChip zchip in zgmDriver.chips)
                    {
                        ...
                        chip = new MDSound.MDSound.Chip
                        {
                            ...
                            Clock = (uint)zchip.defineInfo.clock & 0x7fff_ffff,
                            Option = null
                        };
                        lstChips.Add(chip);
                    ...
                    foreach (Driver.ZGM.ZgmChip.ZgmChip zchip in zgmDriver.chips)
                    {
                        if (!(zchip is Driver.ZGM.ZgmChip.K054539)) continue;
                        ...
                        chipRegister.K054539[zCnt].Use = true;
                        chipRegister.K054539[zCnt].Model = EnmVRModel.VirtualModel;
                        chipRegister.K054539[zCnt].Device = EnmZGMDevice.K054539;
                    }
                ...
                for (int i = 0; i < chipRegister.K054539.Count; i++) if (chipRegister.K054539[i].Use) chipRegister.K054539WriteClock((byte)i, (int)zgmDriver.K054539ClockValue);
        ...
        public static bool vgmPlay(Setting setting)
        {
            ...
            try
            {
                ...
                log.Write("使用チップの調査");
                {
                    ...
                    if (vgmDriver.YM2608ClockValue != 0)
                    {
                        ...
                        for (int i = 0; i < (vgmDriver.YM2608DualChipFlag ? 2 : 1); i++)
                        {
                            chip = new MDSound.MDSound.Chip
                            {
                                ...
                                SamplingRate = 55467,// (UInt32)Common.SampleRate;
                                Volume = setting.balance.YM2608Volume,
                                Clock = vgmDriver.YM2608ClockValue,
                    ...
                    if (vgmDriver.K054539ClockValue != 0)
                    {
                        ...
                        for (int i = 0; i < (((vgm)driver).K054539DualChipFlag ? 2 : 1); i++)
                        {
                            ...
                            if (i == 0)
                            {
                                chipLED.PriK054539 = 1;
                                useChip.Add(EnmChip.K054539);
                            }
                            else
                            {
                                chipLED.SecK054539 = 1;
                            ...
                            hiyorimiDeviceFlag |= 0x2;
                            chipRegister.K054539[i].Use = true;
                            if (chip.Instrument != null) lstChips.Add(chip);
                        }
                    }
                ...
                if (setting.other.sinWaveGen)
                {
                    ...
                    {
                        Option = null,
                        type = MDSound.MDSound.enmInstrumentType.None,
                        Instrument = sin,
                        Update = sin.Update,
                        Start = sin.Start,
                        Stop = sin.Stop,
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    if (chipRegister.K054539[i].Use)
                    {
                        if (chipRegister.K054539[i].Model == EnmVRModel.VirtualModel) useEmu = true;
                        if (chipRegister.K054539[i].Model == EnmVRModel.RealModel) useReal = true;
                    }
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    if (chipRegister.K054539[i].Use) chipRegister.K054539WriteClock((byte)i, (int)vgmDriver.K054539ClockValue);
            ...
            try
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    chip = new MDSound.MDSound.Chip
                    {
                        ...
                        SamplingRate = 55467,// (UInt32)Common.SampleRate;
                        Volume = setting.balance.YM2608Volume,
                        Clock = mubDriver.YM2608ClockValue
                    };
                ...
                for (int i = 0; i < 2; i++)
                {
                    chip = new MDSound.MDSound.Chip
                    {
                        ...
                        SamplingRate = 55467,// (UInt32)Common.SampleRate;
                        Volume = setting.balance.YM2610Volume,
                        Clock = mubDriver.YM2610ClockValue,
                        Option = null
                    };
                ...
                for (int i = 0; i < 1; i++)
                {
                    chip = new MDSound.MDSound.Chip
                    {
                        ...
                        SamplingRate = 55467,// (UInt32)Common.SampleRate;
                        Volume = setting.balance.YM2610Volume,
                        Clock = mubDriver.YM2151ClockValue,
                        Option = null
                    };
                    //chip.Option = new object[] { Common.GetApplicationFolder() };
                    //hiyorimiDeviceFlag |= 0x2;
                ...
                if (!mubDriver.init(
                    ...
                    , mucomManager
                    , chipRegister
                    , [EnmChip.YM2608, EnmChip.S_YM2608, EnmChip.YM2610, EnmChip.S_YM2610, EnmChip.YM2151]
                    , (uint)(Common.SampleRate * setting.LatencyEmulation / 1000)
                    , (uint)(Common.SampleRate * setting.outputDevice.WaitTime / 1000)
                    , mubFileName
                    )
        ...
        private static void softReset(long counter)
        {
            ...
            for (int i = 0; i < chipRegister.K054539.Count; i++) if (chipRegister.K054539[i].Use) chipRegister.K054539SoftReset(counter, i);
        ...
        private static PackData[] MakeSoftResetData()
        {
            ...
            for (int i = 0; i < chipRegister.K054539.Count; i++) if (chipRegister.K054539[i].Use) data.AddRange(chipRegister.K054539MakeSoftReset(i));
        ...
        private static PackData Pack = new();

        private static bool AudioDeviceSync = false;
        private static readonly object lockObjAudioDeviceSync = new();
        ...
        public static bool PlayToWav(Setting setting,string fnWav, bool doSkipStop = false, Action startedOnceMethod = null)
        {
            ...
            Thread mm = new(new ThreadStart(trdToWavRenderingProcess));
            mm.Start();
        ...
        public static bool PlayToMid(Setting setting, string fnMid, bool doSkipStop = false, Action startedOnceMethod = null)
        {
            ...
            Thread mm = new(new ThreadStart(trdToMidRenderingProcess));
            mm.Start();
        ...
        public static bool PlayToVGM(Setting setting, string fnVGM, bool doSkipStop = false, Action startedOnceMethod = null, Document doc=null)
        {
            ...
            toVgmWriter.useChipsFromMub([.. mub]);
            toVgmWriter.SetTagInfo(doc.tag);

            Thread mm = new(new ThreadStart(trdToVGMRenderingProcess));
            mm.Start();
        ...
        private static void playToWavOneProc()
        {
            ...
            {
                EmuSeqCounter++;
                while (EmuSeqCounter > DriverSeqCounter && !driver.Stopped)
                {
                    driver.oneFrameProc();
                }
