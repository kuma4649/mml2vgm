...
namespace mml2vgmIDEx64
{
    public class vgm : baseDriver
    {
                ...
                case 0x80:
                    ...
                    switch (bType)
                    {
                        ...
                        case 0x8c:
                            ...
                            pcmDat.Clear();
                            for (uint i = vgmAdr + 15; i < vgmAdr + 15 + bLen - 8; i++) pcmDat.Add(vgmBuf[i].val);
                            chipRegister.K054539WritePCMData(od, Audio.DriverSeqCounter, chipID, romSize, startAddress, bLen - 8, pcmDat.ToArray(), 0);
        ...
        private void vcSetupStreamControl(outDatum od)
        {
            ...
            chipRegister.DACControlSetupStreamControl(od, Audio.DriverSeqCounter, si, (byte)(chipId & 0x7f), 0, 0, (byte)((chipId & 0x80) >> 7), port, cmd);
            vgmAdr += 5;
        }
